...
public class ReviewStatus {
    ...
    //Build embed.
    public ReviewStatus(Config config, GlobalSQL globalSQL, PlotSQL plotSQL, RegionSQL regionSQL, Discord discord, Scheduler scheduler) {
        ...
        //Get channel.
        supportInfoChannel = discord.getSupportInfoChannel();
        supportChatChannel = discord.getSupportChatChannel();

        //Try to get the message ID from config, if it does not exist, create a new message id.
        messageID = config.getString("message.reviewer");
        ...
        scheduler.createRepeatingTask(() -> {
                ...
                for (int id : plots) {
                    ...
                    if (time - (3 * day) > submit_time) {
                        ...
                        supportChatChannel.sendMessage("<@&" + discord.getReviewerRoleID() + "> Plot " + id + " has been submitted for " + days + " days, please " + textStatus + " it as soon as possible!").queue();
    ...
    private void createMessage() {
        ...
        try {
            ...
            log.severe("An error occurred while fetching region requests: " + e.getMessage());
        }
    ...
    private MessageEmbed getEmbed() throws SQLException {
        ...
        eb.setFooter("Last updated: " + org.btuk.proxy.core.utils.Time.getDateTime(org.btuk.proxy.core.utils.Time.currentTime()));
