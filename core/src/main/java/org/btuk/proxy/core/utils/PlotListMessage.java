...
public class PlotListMessage {
    ...
    /**
     * @param globalSQL    {@link GlobalSQL} instance.
     * @param plotSQL      {@link PlotSQL} instance.
     ...
     */
    public PlotListMessage(GlobalSQL globalSQL, PlotSQL plotSQL, String type, String title, String query, String emptyMessage, int page, String player) {
        ...
        if (player != null) {
            if (globalSQL.hasRow("SELECT uuid FROM player_data WHERE name='" + player + "';")) {
                String uuid = globalSQL.getString("SELECT uuid FROM player_data WHERE name='" + player + "';");
        ...
        plots = plotSQL.getIntList(query);
    ...
    public MessageEmbed createList() {
        ...
        do {
            ...
            if (plotSQL.hasRow("SELECT id FROM plot_members WHERE id=" + plots.get(index) + " AND is_owner=1;")) {
                plot_message.append(" - claimed by: ").append(Discord.escapeDiscordFormatting(globalSQL.getString("SELECT name FROM player_data WHERE uuid='" +
                        plotSQL.getString("SELECT uuid FROM plot_members WHERE id=" + plots.get(index) + " AND is_owner=1;") + "';")));
            }

            //If the plot is completed, add the builder.
            if (plotSQL.hasRow("SELECT id FROM plot_data WHERE id=" + plots.get(index) + " AND status='completed'")) {
                plot_message.append(" - completed by: ").append(Discord.escapeDiscordFormatting(globalSQL.getString("SELECT name FROM player_data WHERE uuid='" +
                        plotSQL.getString("SELECT uuid FROM plot_review WHERE plot_id=" + plots.get(index) + " AND accepted=1 AND completed=1;") + "';")));
            }

            //If the plot is currently being reviewed add that.
            if (plotSQL.hasRow("SELECT 1 FROM plot_submission WHERE plot_id=" + plots.get(index) + " AND status<>'submitted';")) {
                plot_message.append(" (")
                        .append(plotSQL.getString("SELECT status FROM plot_submission WHERE plot_id=" + plots.get(index) + ";"))
                        .append(")");
            }
