...
public class SubmittedPlots extends PlotListCommand {
    ...
    public SubmittedPlots(String name, String description, OptionData... options) {
        super(name, description, "Submitted Plots", "SELECT pd.id FROM plot_data AS pd INNER JOIN plot_members AS u ON pd.id=u.id WHERE (pd.status='submitted') AND u.is_owner=1%uuid%;", "There are currently no submitted plots.", options);
    }
}
